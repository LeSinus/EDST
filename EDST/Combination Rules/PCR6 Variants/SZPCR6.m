function [Combined_Masse]=SZPCR6(MasseIn)
    ...
    conjunctive_distance_missing_masses=repmat(0,1,power_set_size);
     ...
     for i=2:power_set_size
        ...
        total = 0;
        for j=1:rows
            Zhang_Distance = get_Zhang_Distance(conjunctive_Element_Matrix(j,:));
            product =1;
            for k=1:cols
        ...
        % Calculate conjunctive distance missing masses
         conjunctive_distance_missing_masses = conjunctive_distance_missing_masses +   get_Missing_Mass(MasseIn, conjunctive_Element_Matrix,2);
    end
